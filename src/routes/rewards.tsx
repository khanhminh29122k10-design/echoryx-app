...
function Rewards() {
  ...
  async function redeem(itemId: string) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't request that reward.");
    }
  }
  ...
  return (
    <MobileFrame>
      <ScreenHeader title="Rewards" right={<Trophy className="w-5 h-5 text-warning" />} />

      <div className="relative rounded-3xl p-5 gradient-primary shadow-glow overflow-hidden">
        <div className="absolute -right-6 -top-6 opacity-20">
          <Gift className="w-32 h-32 text-white" />
        </div>
        <div className="text-primary-foreground/80 text-xs">Star balance</div>
        ...
        {nextBadge && (
          <div className="text-primary-foreground/80 text-[11px] mt-4">Next up: "{nextBadge.name}" — {nextBadge.description}</div>
        )}
      </div>

      {pending.length > 0 && (
        <div className="mt-4">
          <h2 className="font-bold mb-2 text-sm">Pending approval</h2>
            ...
            {pending.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-3 p-3 rounded-2xl bg-card border border-warning/40">
                  <div className="flex-1 text-sm font-semibold">{item?.name ?? "Reward"} — {r.costStars} ⭐</div>
                  <button onClick={() => decide(r.id, "approved")} className="text-xs font-semibold text-success">Approve</button>
                  <button onClick={() => decide(r.id, "rejected")} className="text-xs font-semibold text-destructive">Reject</button>
                </div>
              );
            })}
          </div>
      ...
      <h2 className="font-bold mt-6 mb-3">Badges</h2>
      ...
      <h2 className="font-bold mt-6 mb-3">Redeem</h2>
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex items-center gap-3 p-3 rounded-2xl bg-card border border-border/60">
              <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center"><Gift className="w-5 h-5 text-primary-glow" /></div>
              <div className="flex-1 text-sm font-semibold">{item.name}</div>
              <span className="text-xs font-bold flex items-center gap-1">{item.costStars} <Star className="w-3 h-3 fill-warning text-warning" /></span>
              {alreadyPending ? (
                ...
                <button
                  ...
                  className="text-xs font-semibold text-primary-glow disabled:text-muted-foreground disabled:opacity-60"
                >
                  Redeem
                </button>
              )}
